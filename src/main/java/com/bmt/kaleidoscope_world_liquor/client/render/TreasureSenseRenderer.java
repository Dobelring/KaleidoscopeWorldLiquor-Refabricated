 ...
 *
 * 穿墙实现（2026-09-10，照 Fabric 官方文档 1.21.11 "Rendering in the World"
 * 穿墙 waypoint 示例）：
 ...
 *   深度状态随管线走，原版与光影（Iris fallback shader）下均可穿墙。
 * - LINES 顶点格式带 LineWidth 属性（rendertype_lines.vsh），每顶点必须 setLineWidth。
 * - Iris 兼容：自定义管线需经 IrisPipelines.assignPipeline 绑定到 ShaderKey.LINES，否则
 *   Iris 对未知管线走 FAKE_FUNCTION（shader 为空被跳过）；绘制时机用 BEFORE_TRANSLUCENT
 *   （官方示例时机，处于 Iris 重定向路径内，END_MAIN 直画会被最终合成覆盖）。
 */
...
public final class TreasureSenseRenderer {
    ...
    /** 提取：把容器 AABB 的 12 条边写入 BufferBuilder（世界坐标-相机，与 context.matrices 相机相对一致） */
    private static void extractBoxes(net.fabricmc.fabric.api.client.rendering.v1.world.WorldRenderContext context) {
