...
import com.bmt.kaleidoscope_world_liquor.mixin.accessor.CreativeModeTabAccessor;
 ...
 *
 * <p>原版创造界面按每个 tab 自己的 row()/column() 摆放按钮，而 Fabric 的 fabric-creative-tab-api-v1
 * 会在 CreativeModeTabs.buildAllTabContents 尾部按命名空间给所有非原版 tab 重新分配 page/row/column（分页），
 * 结果固定为 tavern_deco、tavern_main、本模组 tab —— 所以只重排列表改不了视觉位置，必须在位置层落位。
 *
 * <p>做法：取这三个 tab 当前占用的槽位，按 [本模组, tavern_main, tavern_deco] 的顺序重新填入这些槽位。
...
public final class CreativeTabOrder {
    ...
    public static void apply() {
        ...
        if (slotSharedWithOtherMod(wanted, slots)) {
            // 槽位被别的非原版 tab 占用 → 说明这三个 tab 被分到了不同页（不同页可共用同一 row/column），换位会重叠
            return;
        }
